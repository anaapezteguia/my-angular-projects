...
const URL: string =
  'https://ejemplos-dc1c1.firebaseio.com/angular-sngular2/tareas/anaam';
...
export class TareasService {
  ...
  getTareas(): Observable<any> {
    return (
      this.http
        .get(`${URL}.json`)
        // transformamos la respuesta en array
        .pipe(
          map((data: any) => {
            const arrTareas = [];
            for (let id in data) {
  ...
  deleteTarea(id: string): Observable<null> {
    return this.http.delete<null>(`${URL}/${id}.json`).pipe(
      tap(() => {
  ...
  completeTarea(id: string, completada: boolean): Observable<any> {
    return this.http.patch(`${URL}/${id}.json`, { completada: completada });
  }
}
